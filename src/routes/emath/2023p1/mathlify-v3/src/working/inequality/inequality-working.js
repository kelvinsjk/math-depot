...
import { Fraction, Expression } from '../../core/index.js';
...
export class InequalityWorking {
	...
	times(x, options) {
		...
		this.lhs = newLHS;
		this.rhs = newRHS;
		if ((typeof x === 'number' && x < 0) || (x instanceof Fraction && x.is.negative())) {
	...
	factorizeQuadratic(options) {
		insertIntertext(this, options);
		const intervals = solveQuadraticInequality(this.lhs, this.sign, this.rhs, options);
	...
	combineRationalTerms(options) {
		...
		const newRHS =
			simplifiedRHS instanceof RationalTerm
				...
				: simplifiedRHS;
		if (simplifiedLHS instanceof RationalTerm || simplifiedRHS instanceof RationalTerm) {
			insertIntertext(this, options);
			this.lhs = newLHS;
			this.rhs = newRHS;
			if (!options?.hide) {
	...
	crossMultiply(options) {
		...
		if (simplifiedLHS instanceof RationalTerm) {
			if (simplifiedRHS instanceof RationalTerm) {
				...
				const newLHS = simplifiedLHS.num.times(simplifiedLHS.coeff).times(rhs.den);
			...
			if (simplifiedRHS instanceof RationalTerm) {
				if (simplifiedLHS.is.constant()) {
					...
					const newRHS = simplifiedRHS.num.times(lhs.den).times(simplifiedRHS.coeff);
					this.lhs = newLHS;
					this.rhs = newRHS;
					if (!options?.hide) {
						this.lhsArray.push(newLHS);
	...
	toString() {
		return this.lhsArray.reduce((prev, lhs, i) => {
			const equal = this.aligned ? ` &${this.signArray[i]} ` : ` ${this.signArray[i]} `;
			const newLine = i === 0 ? '' : '\n\t\\\\ ';
			const equalSign =
