...
<section
	...
	aria-labelledby={answersName}
	class:leading-6={solutionMode}
>
	<h2 id={answersName} class:mt-4={!solutionMode} class="dark:text-zinc-300">
		{solutionMode ? 'Full solutions' : 'Answers'}
	</h2>
	<div
		class="parts grid gap-y-4 answer-container"
		class:solutions={solutionMode}
		class:answers={!solutionMode}
	>
		...
		{#if answer['parts'] !== undefined}
			{#each answer.parts as part, i}
				{#if part['body'] !== undefined}
					{#if solutionMode}
						<div class="not-prose">
							<h3 class="font-semibold text-right dark:text-zinc-300">
					...
					{:else}
						<div class="not-prose">
							<h3 class="font-semibold text-right dark:text-zinc-300">
								<a class="link" href={`#solutions-part-${part['partNo'] ?? i + 1}`}>
									({qnLabels[answer['partLabelType'] ?? 'alpha'][
										part['partNo'] ?? i + 1
									]})
								</a>
							</h3>
					...
					{#each part.parts as subpart, j}
						{#if solutionMode}
							<div class="font-semibold text-right dark:text-zinc-300">
								({qnLabels[answer['partLabelType'] ?? 'alpha'][
									part['partNo'] ?? i + 1
								]}{qnLabels[part['partLabelType'] ?? 'roman'][
									subpart['partNo'] ?? j + 1
								]})
							</div>
						{:else}
							<a
								class="font-semibold text-right dark:text-zinc-300"
