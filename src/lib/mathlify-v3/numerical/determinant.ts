...
export function determinantFrac(...args: (number | Fraction)[]): Fraction {
	...
	if (argsFrac.length === 4) {
		...
		const a = argsFrac[0].times(
			determinantFrac(argsFrac[4], argsFrac[5], argsFrac[7], argsFrac[8]),
		);
		const b = argsFrac[1].times(
			determinantFrac(argsFrac[3], argsFrac[5], argsFrac[6], argsFrac[8]),
		);
		const c = argsFrac[2].times(
			determinantFrac(argsFrac[3], argsFrac[4], argsFrac[6], argsFrac[7]),
		);
