...
export function cramers(...args: number[]): number[] {
	if (args.length === 6) {
		...
		throw new Error(
			'only 2x2 (6 arguments) and 3x3 (12 arguments) equations are supported',
		);
	}
}
 ...
 *
 */
function cramersFrac4x4(
	...args: (Fraction | number)[]
): [Fraction, Fraction, Fraction, Fraction] {
	if (args.length !== 20) {
