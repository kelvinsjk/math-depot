...
export class Laurent extends Expression {
	...
	constructor(
		...
		negCoeffs = negCoeffs ?? [0];
		const negCoeffsFrac = negCoeffs.map((x) => numberToFraction(x));
		while (negCoeffsFrac.at(-1)?.isEqualTo(0) && negCoeffs.length > 1) {
			negCoeffsFrac.pop();
		}
	...
	/**
	 * combines entire series as a fraction
	 * num / x^n where -n is the most negative power encountered
	 * in this series,
	 * and returns num as a Polynomial
	 */
	toNumeratorPolynomial(ascending = false): Polynomial {
		const negCoeffs = [...this.negCoeffs];
		negCoeffs.reverse();
		const coeffs = [...negCoeffs, ...this.poly.coeffs];
		if (!ascending) {
			coeffs.reverse();
		}
		return new Polynomial(coeffs, { ascending });
	}
